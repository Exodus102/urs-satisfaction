...
<div class="hidden lg:flex lg:w-3/5 xl:w-[70rem] bg-cover bg-center flex-col justify-around p-10 lg:p-12"
  ...
  $logo_path_header_2fa = '../../resources/img/new-logo.png'; // Default fallback
  try {
    // We need a separate DB connection here as this file is included early.
    // Using a new variable name to avoid conflicts if other headers are used.
    $pdo_header_2fa = new PDO("mysql:host=localhost;dbname=db_css", "root", "");
    $pdo_header_2fa->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

    $stmt_header_2fa = $pdo_header_2fa->query("SELECT logo_path FROM tbl_logo WHERE status = 1 LIMIT 1");
    $active_logo_header_2fa = $stmt_header_2fa->fetchColumn();
    if ($active_logo_header_2fa) {
      $logo_path_header_2fa = '../../' . $active_logo_header_2fa;
    }
  } catch (PDOException $e) {
    // On error, the default logo is used.
  }
  ?>
  <img src="<?php echo htmlspecialchars($logo_path_header_2fa); ?>" alt="URS Logo" class="size-32 object-contain">
  <div class="">
